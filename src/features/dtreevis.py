...
def dtreeviz_analysis(X_train_clean, y_train_clean, target_column_name, model, class_names=[0, 1],
                      entire_tree=True, instance_to_query=None, nodes_to_query=None):
    '''
    this function recives the training data and target, a trained model and a class name (as a list, default is [0,1]).
    it returns a dtreeviz visualisation for the model.
    if entire_tree = True, it will return the entire tree.
    if nodes_to_query is a list of two int, it will also return this specific section of the tree (root level is 0).
    ...
    viz_model = dtreeviz.model(model, tree_index=1,
                               # tree_index specifies which individual tree within the ensemble to visualize.
                               X_train=X_train_clean, y_train=y_train_clean,
                               feature_names=list(X_train_clean.columns),
                               target_name=target_column_name, class_names=[class_names[0], class_names[
            1]])  # class_names=['no', 'yes'] can be int of string
